...
import com.jstech.zhiyutong.weight.GlideCircleTransform;
import com.jstech.zhiyutong.weight.GlideRoundTransform;
...
public class GlideUtils implements ImageLoader{
    ...
    public static synchronized GlideUtils getInstance(int flag){
        if (glideUtils == null){
            glideUtils = new GlideUtils(flag);
        }
        return glideUtils;
    }
...
//        GlideApp.with(mContext).load(path).asBitmap().into(mImageView);
    ...
    public void displayImage(Context context, String path, ImageView imageView) {
        GlideUtils.getInstance(0).loadImageView(context,path,imageView);
    }
    ...
    public static void displayCircleImage(Context context, String path, ImageView imageView) {
        GlideUtils.getInstance(1).loadImageView(context,path,imageView);
    }
